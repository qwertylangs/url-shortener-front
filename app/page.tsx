...
export default function Home() {
  ...
  const handleDeleteUrl = async (alias: string, id: number) => {
    if (!confirm(`Вы уверены, что хотите удалить алиас "${alias}"?`)) {
      return;
    }

    setDeletingId(id);
    try {
      const response = await api.deleteUrl(alias);
      ...
      if (response.ok) {
        ...
        const data = await response.json().catch(() => ({}));
        alert(data.error || 'Ошибка удаления алиаса');
      }
    } catch (err) {
              ...
              {urls.map((url) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <button
                      onClick={() => handleDeleteUrl(url.alias, url.id)}
                      disabled={deletingId === url.id}
                      className="flex-shrink-0 p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Удалить алиас"
                    >
                      {deletingId === url.id ? (
